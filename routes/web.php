...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['role:cs_staff|cs_leader|admin'])
        ->prefix('office')
        ->name('office.')
        ->group(function () {
            ...
            Route::get('/orders/{order}/stock-order', [\App\Http\Controllers\CS\OrderManagementController::class, 'stockOrder'])->name('orders.stock-order');
        });
    ...
    // CATALOG MANAGEMENT
    Route::resource('catalogs', \App\Http\Controllers\CatalogController::class)->middleware(['auth', 'verified']);

    // Fulfills Addendum Section 3.a & 3.c: Company Management Module
    // This resolves the Route [companys.index] not defined error.
    Route::resource('companys', \App\Http\Controllers\CompanyController::class)->middleware(['auth', 'verified']);
