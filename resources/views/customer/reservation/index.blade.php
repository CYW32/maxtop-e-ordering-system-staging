    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (!$draft || $items->isEmpty())
                {{-- Empty State: Fulfills Section 4.a.1 --}}
            ...
            @else
                @php
                    // ARCHITECTURE: Group by item_id to handle multiple UOMs per SKU [1, 2]
                ...
                <div class="bg-white shadow-sm sm:rounded-[2.5rem] border border-gray-100 overflow-hidden">
                                ...
                                @foreach ($entries as $orderItem)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            {{-- UNIT FIX: Distinguishes between UOM and Individual Unit [v1.4 Bugfix] --}}
                                            <span
                                                class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight {{ $orderItem->uom_id ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-gray-100 text-gray-500 border border-gray-200' }}">
                                                {{ $orderItem->uom?->uom_name ?? __('Individual Unit') }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <form method="POST" action="{{ route('reservation.update', $orderItem) }}"
                                                ...
                                                <input type="number" name="quantity"
                                                    ...
                                                    class="w-20 text-center border-gray-200 rounded-xl text-sm font-black focus:ring-blue-500 focus:border-blue-500">
                    ...
                    {{-- Footer: Price Blind Total QTY and Submission [4.b, 456] --}}
                    <div class="p-10 bg-gray-900 flex flex-col md:flex-row justify-between items-center gap-8">
                        ...
                        <form method="POST" action="{{ route('reservation.submit') }}"
                            ...
                            @csrf
                            <button type="submit"
                                class="bg-green-600 hover:bg-green-700 text-white py-5 px-14 rounded-2xl text-sm font-black uppercase shadow-xl shadow-green-900/40 transition-all hover:-translate-y-1 active:translate-y-0 active:scale-[0.98]">
                ...
                {{-- Policy Note: Enforces Price Blind Policy visual context --}}
                <div class="flex items-center justify-center gap-2 text-gray-400">
                    ...
                    <span
                        class="text-[9px] font-black uppercase tracking-widest">{{ __('Reservation only: Prices are validated during the CS approval process') }}</span>
                </div>
            @endif
        </div>
    </div>
